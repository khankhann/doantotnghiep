...
import { toast } from "sonner"; // Giả sử fen có dùng sonner để thông báo
...
function CreateProductPage() {
  ...
  const [productData, setProductData] = useState({
    ...
    gender: "",
    images: [],
    rfid: "", // 🔥 THÊM STATE ĐỂ LƯU MÃ RFID
  });
  ...
  // 🔥 XỬ LÝ SỰ KIỆN QUÉT RFID
  const handleRFIDKeyDown = (e) => {
    // Máy quét RFID thường tự động gửi phím 'Enter' sau khi quét xong mã
    if (e.key === "Enter") {
      e.preventDefault(); // Chặn form submit trang
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Bắn API tạo product kèm theo mã RFID
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-8">
        ...
        {/* CỘT TRÁI: FORM NHẬP THÔNG TIN (Chiếm 2/3) */}
        <div className="w-full lg:w-2/3 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
          ...
          <div className="pt-4 border-t mt-6">
            ...
            <div className="flex items-center gap-4">
               <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-lg transition-colors border border-gray-300">
                  Chọn ảnh tải lên
                  <input type="file" onChange={handleImageUpload} className="hidden" />
               </label>
               {uploading && <span className="text-sm text-blue-600 font-bold animate-pulse">Đang tải ảnh...</span>}
            </div>
            
            <div className="flex gap-4 mt-4 flex-wrap">
              {productData.images.map((img, index) => (
                <div key={index} className="relative group">
                  <img src={img.url} alt="Product" className="w-24 h-24 object-cover rounded-xl shadow-md border" />
              ...
              <div className="flex flex-col items-center text-center">
                 ...
                 <div className="w-full relative">
                    <input 
                      ...
                      onKeyDown={handleRFIDKeyDown} // Chặn Enter submit form
