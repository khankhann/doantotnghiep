...
useEffect(()=>{
  ...
  const handlePaymentSuccess = async(detail) => {
    try{
      const response = await api.put(`${import.meta.env.VITE_BACKEND_URL}/api/${checkoutId}/pay`, {
        paymentStatus : "paid", 
    ...
    const handleFinalizeCheckout = async(checkoutId)=>{
      try{
        const response = await api.post(`${import.meta.env.VITE_BACKEND_URL}/api/checkout/${checkoutId}/finalize`,{}, {
          headers : {
            Authorization : `Bearer ${localStorage.getItem("userToken")}`
          }
        })
